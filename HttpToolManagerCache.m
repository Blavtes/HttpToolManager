...
static NSString * const kHTMNetManagerCache = @"HTMNetManagerCache";
static  YYCache *_dataCache;

@implementation HttpToolManagerCache

+ (void)initialize
{
    _dataCache = [YYCache cacheWithName:kHTMNetManagerCache];
    //    _dataCache.memoryCache.ageLimit = 199;
}
 ...
 */
+ (void)htm_setHttpCache:(id)httpData
              urlString:(NSString *)urlString
             parameters:(NSDictionary *)parameters
{
    NSString *cacheKey = [self htm_cacheWithUrlString:urlString parameters:parameters];
...
/**
 ...
 @return 缓存的服务器数据
 */
+ (id)htm_httpCacheWithUrlString:(NSString *)urlString
                     parameters:(NSDictionary *)parameters
{
    NSString *cacheKey = [self htm_cacheWithUrlString:urlString parameters:parameters];
 ...
 */
+ (void)htm_httpCacheWithUrlString:(NSString *)urlString
...
+ (NSString *)htm_cacheWithUrlString:(NSString *)urlString
                         parameters:(NSDictionary *)parameters
{
    if (!parameters)
    {
...
/**
 ...
 @return 总对象的大小（以M为单位）。
 */
+ (CGFloat)htm_getAllHttpCacheSize
{
